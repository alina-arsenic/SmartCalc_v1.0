...
void calculator_initialization() {
  ...
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 6; j++) {
      gtk_grid_attach(GTK_GRID(numpad.grid), numpad.button[i * 6 + j], j, i + 1,
                      1, 1);
    }
  }
  ...
  gtk_widget_set_size_request((GtkWidget *)numpad.drawingArea, DISPLAY_WIDTH,
                              DISPLAY_HEIGHT);
  memset(numpad.expr, 0, MAX_LEN);
  g_signal_connect((GtkWidget *)numpad.drawingArea, "draw", G_CALLBACK(on_draw),
                   NULL);
  gtk_grid_insert_row(GTK_GRID(numpad.grid), 0);
  gtk_grid_attach(GTK_GRID(numpad.grid), (GtkWidget *)numpad.drawingArea, 0, 0,
                  7, 1);
  ...
  gtk_label_set_text(GTK_LABEL(numpad.label),
                     "Welcome to the Smart Calc by weaveryu!");
}
...
void credit_initialization() {
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(credit.scrolled_window),
                                 GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);

  gtk_widget_set_size_request(credit.scrolled_window,
                              APP_WIDTH - BORDER_SIZE * 2,
                              APP_HEIGHT * 3 / 4 - BORDER_SIZE);
  ...
  g_signal_connect(credit.button[0], "clicked", G_CALLBACK(credit_calculate),
                   NULL);
...
void deposit_initialization() {
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(in_window),
                                 GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(out_window),
                                 GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_widget_set_size_request(out_window, APP_WIDTH / 2, APP_HEIGHT / 6);
  gtk_grid_attach(GTK_GRID(deposit.grid), out_window, 1, 8, 2, 1);

  deposit.message =
      gtk_label_new("The format of list entries is: 'dd.mm.yyyy  summ'");
  ...
  g_signal_connect(deposit.button[0], "clicked", G_CALLBACK(deposit_reset),
                   NULL);
  g_signal_connect(deposit.button[1], "clicked", G_CALLBACK(deposit_calculate),
                   NULL);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(result_window),
                                 GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
  gtk_widget_set_size_request(result_window, APP_WIDTH - BORDER_SIZE * 2,
                              APP_HEIGHT / 8);
