...
int check_limits(s21_limits *lims) {
	...
	if (parse_double(lims->x1_buffer, &lims->x1) || parse_double(lims->x2_buffer, &lims->x2) ||
		parse_double(lims->y1_buffer, &lims->y1) || parse_double(lims->y2_buffer, &lims->y2)) {
		return 3;
	}
			...
			if (strchr(input_buffer, 'x')) {
				...
				if (code == 0) {
					...
					if (code == 0) {
						...
						gtk_label_set_text(GTK_LABEL(numpad.label), "No more than 15 'x' please");
					}
                    ...
                    make_plot();
					if (code == 1) {
						gtk_label_set_text(GTK_LABEL(numpad.label), "Please specify limits");
					} else if (code == 2) {
						gtk_label_set_text(GTK_LABEL(numpad.label), "Values in limits must be from -1000000 to 1000000");
					} else if (code == 4) {
						gtk_label_set_text(GTK_LABEL(numpad.label), "Values in limits must be different");
					} else {
						gtk_label_set_text(GTK_LABEL(numpad.label), "Incorrect symbols in limits");
					}
				}
			} else {
